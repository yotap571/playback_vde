...
function initMediaSource() {
  ...
  if (!window.MediaSource) {
    ...
    document.getElementById("incomingMsgOutput").value +=
      "error: No Media Source API available" + "\r\n";
    return;
  }
  ...
  if (!MediaSource.isTypeSupported(mimeCodec)) {
    ...
    document.getElementById("incomingMsgOutput").value +=
      "error: Unsupported MIME type or codec" + "\r\n";
    return;
  }
  ...
  function loadPacket() {
    // called when sourceBuffer is ready for more
    if (!sourceBuffer.updating) {
      if (queue.length > 0) {
        data = queue.shift(); // pop from the beginning
        appendToBuffer(data);
      } else {
        // the queue runs empty, so we must force-feed the next packet
...
function openWSConnection(protocol, hostname, port, endpoint) {
  ...
  try {
    ...
    // Define your imports object
    let imports = {
      env: {
        // Provide all necessary imported functions here.
        // For example, if the module imports a function named 'myImportedFunction', you might do something like this:
        myImportedFunction: function () {
          // Implementation of myImportedFunction.
        },
      },
    };

    loadWebAssembly("./libcmsv6decode.wasm").then((instance) => {
